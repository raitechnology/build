...
After verifying newuser1 is ok and is not a replay of old session by checking
the *newuser1.NONCE*, *seqno* and the *utc_usecs* compared with the system
clock, a *HMAC* is computed with the information from the subscriber and the
publisher, creating a trust bond from *publisher1* to *newuser1*.
...
subscriber has a record of sending it with the *CNONCE*.  *Newuser1* is still
